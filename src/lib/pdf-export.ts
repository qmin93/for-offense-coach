...
import { formatDistance } from "@/domain/dsl/types";
...
// ============================================
// Render Stabilization Utilities
// ============================================
...
/**
 * Wait for render to stabilize (2 animation frames)
 * This ensures CSS transitions, transforms, and SVG renders are complete
 */
async function waitForRenderStabilization(): Promise<void> {
...
/**
 * Full render preparation before export
 */
...
// Default export scale (3x for print quality)
...
async function svgToDataUrl(
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      canvas.width = width * scale; // Scale for print quality
...
function addScoutCardPage(
  ...
  if (pageStyle === "classic") {
    ...
    const boxes = [
      ...
      { label: "DISTANCE", value: play.meta?.distance ? formatDistance(play.meta.distance) : "-" },
...
function addAttribution(pdf: jsPDF) {
  pdf.setFontSize(7);
  pdf.setTextColor(180, 180, 180);
  pdf.text(
    "Generated by ForOffenseCoach",
    PAGE_WIDTH / 2,
    PAGE_HEIGHT - 10,
    { align: "center" }
  );
...
export async function exportPlaybookToPdf(
  ...
  // Wait for fonts and render to stabilize before export
...
export async function capturePlaySvgAsImage(
  ...
  // Wait for render to stabilize before capture
  await waitForRenderStabilization();
  ...
  try {
    // 3x scale is applied inside svgToDataUrl for print quality
    return await svgToDataUrl(svgElement as SVGSVGElement, 800, 600);
  } catch (e) {
    console.warn("[Export] Failed to capture SVG:", e);
    return null;
  }
}
